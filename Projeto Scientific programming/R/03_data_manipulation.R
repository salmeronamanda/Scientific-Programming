...
# The same operation without specifying column names
...
# Doing the same operation with left_join
...
# Extra: filter, select  -------------------------------------------------------
# Filtering based on logical test
## using dplyr::filter
sp1 <- filter(comm_total, TaxCode == "sp1", Elev > 3)
## using base
sp1_base <- comm_total[comm_total$TaxCode == "sp1" & comm_total$Elev > 3, ]

# Selecting specific rows
## using dplyr::select
comm_select <- select(comm_total, Sites, TaxCode, Abundance)
## using base
comm_base <- comm_total[, c("Sites", "TaxCode", "Abundance")]
